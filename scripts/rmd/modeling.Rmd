...
## RQ 3
Our third reserach question corresponds to differences in absenteeism rates between schools for students who are and are not classified as economically disadvantaged. This implies modeling the interaction between days absent and economic disadvantaged status, and then allowing those coefficients to randomly vary between schools.
...
Next let's see how this varies by economically disadvantaged status.

```{r}
m1_abs <- glmer(
  days_absent + 1 ~ econ_dis_stable +
    (1 | ssid) + (1 | schid),
  data = d,
  family = Gamma(link = "log"),
  control = glmerControl(optimizer = "Nelder_Mead")
